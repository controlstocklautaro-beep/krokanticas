...
export function CustomersModule({ businessId }: { businessId: string }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { businessId, id: typeof editing === "object" && editing ? editing.id : undefined, name: form.get("name"), phone_number: form.get("phone"), email: form.get("email"), notes: form.get("notes") };
    try {
  ...
  const filtered = contacts.filter((contact) => `${contact.name} ${contact.phone_number} ${contact.email || ""}`.toLowerCase().includes(search.toLowerCase()));
  return <div className="real-module"><div className="module-heading"><div><span className="eyebrow">BASE DE CLIENTES</span><h1>Clientes</h1><p>Información sincronizada con conversaciones y pipeline.</p></div><button className="primary" onClick={() => setEditing("new")}>＋ Nuevo cliente</button></div>{error && <button className="module-error" onClick={() => setError("")}>{error} ×</button>}<div className="panel table-panel"><div className="table-tools"><label className="search"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, teléfono o email" /></label><span className="record-count">{filtered.length} clientes</span></div><div className="data-table"><div className="data-row contacts-grid data-head"><span>CLIENTE</span><span>TELÉFONO</span><span>EMAIL</span><span>BOT</span><span>ACCIONES</span></div>{filtered.map((contact) => <div className="data-row contacts-grid" key={contact.id}><span className="customer-cell"><i className="guest-avatar">{initials(contact.name)}</i><strong>{contact.name}</strong></span><span>{contact.phone_number}</span><span>{contact.email || "—"}</span><span><b className={contact.agent_active ? "state-pill on" : "state-pill off"}>{contact.agent_active ? "Bot ON" : "Bot OFF"}</b></span><span className="row-actions"><button onClick={() => setEditing(contact)}>Editar</button><button onClick={() => remove(contact)}>Eliminar</button></span></div>)}{!filtered.length && <div className="empty-table">No se encontraron clientes.</div>}</div></div>{editing && <div className="modal-backdrop" onMouseDown={() => setEditing(null)}><form className="modal" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">CLIENTES</span><h2>{editing === "new" ? "Nuevo cliente" : "Editar cliente"}</h2></div><button type="button" onClick={() => setEditing(null)}>×</button></div><label>Nombre<input name="name" defaultValue={editing === "new" ? "" : editing.name} required autoFocus /></label><label>Teléfono<input name="phone" defaultValue={editing === "new" ? "" : editing.phone_number} required /></label><label>Email<input name="email" type="email" defaultValue={editing === "new" ? "" : editing.email || ""} /></label><label>Notas<textarea name="notes" defaultValue={editing === "new" ? "" : editing.notes || ""} /></label><div className="modal-actions"><button type="button" className="secondary" onClick={() => setEditing(null)}>Cancelar</button><button type="submit" className="primary">Guardar cliente</button></div></form></div>}</div>;
}
