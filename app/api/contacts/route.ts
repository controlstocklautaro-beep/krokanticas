...
export async function GET(req: Request) {
  try {
    ...
    const result = await getD1().prepare(`
      SELECT id, phone_number, name, email, notes, agent_active, created_at, updated_at
...
export async function POST(req: Request) {
  try {
    const body = await req.json() as { businessId?: string; name?: string; phone_number?: string; email?: string; notes?: string };
    ...
    await db.batch([
      db.prepare(`
        INSERT INTO contacts (id, business_id, phone_number, name, email, notes, agent_active, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)
      `).bind(id, businessId, phoneNumber, name, body.email?.trim() || null, body.notes?.trim() || null, now, now),
      db.prepare(`
...
export async function PATCH(req: Request) {
  try {
    const body = await req.json() as { businessId?: string; id?: string; name?: string; phone_number?: string; email?: string; notes?: string };
    const businessId = businessIdFrom(req, body.businessId);
    await requireBusinessAccess(req, businessId, { roles: ["owner", "admin", "manager", "reception"] });
    if (!body.id) throw new ApiError("Falta id", 400);
    const current = await getD1().prepare("SELECT phone_number, name, email, notes FROM contacts WHERE id = ? AND business_id = ?")
      .bind(body.id, businessId).first<{ phone_number: string; name: string; email: string | null; notes: string | null }>();
    ...
    const db = getD1();
    await db.batch([
      db.prepare("UPDATE contacts SET phone_number = ?, name = ?, email = ?, notes = ?, updated_at = ? WHERE id = ? AND business_id = ?")
        .bind(phoneNumber, name, body.email ?? current.email, body.notes ?? current.notes, Date.now(), body.id, businessId),
